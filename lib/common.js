...
async function anthropicSummarize(prompt, model = 'claude-sonnet-4-6') {
 ...
 */
// === slug generation (matches site's name_city_state convention) ===
const STATE_ABBREV_FOR_SLUG = {
...
function _slugClean(s) {
  return (s || '').toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
}
function generateSlug(name, city, state) {
  if (!name) return null;
  const stateRaw = (state || '').toLowerCase().trim();
  const stateAbbr = STATE_ABBREV_FOR_SLUG[stateRaw] || _slugClean(state);
  const parts = [_slugClean(name), _slugClean(city), stateAbbr].filter(Boolean);
...
module.exports = {
  ...
  downloadAndStoreImage, validateShopData, auditShopImages, generateSlug, parseFencedJSON
};
